...
public class PlayerFlightControl : MonoBehaviour
{
	...
	[HideInInspector] public float roll, yaw, pitch; //Inputs for roll, yaw, and pitch, taken from Unity's input system.
	[HideInInspector] public bool afterburner_Active = false; //True if afterburners are on.
	[HideInInspector] public bool slow_Active = false; //True if brakes are on
	...
	public void fireShot() {
		...
		if (missile == null)
		{
			Debug.LogError("(FlightControls) Missile GameObject is null!");
			return;
		}
		if (Camera.main == null) {
		...
		if(weponNum == 0)
		{
			shotObj = (GameObject)GameObject.Instantiate(bullet, weapon_hardpoint_1.position, Quaternion.identity);
		}
		else
		{
			shotObj = (GameObject)GameObject.Instantiate(missile, weapon_hardpoint_1.position, Quaternion.identity);
		}
		...
		if (Physics.Raycast(vRay, out hit)) {
			shotObj.transform.LookAt(hit.point);
			shotObj.GetComponent<Rigidbody>().AddForce((shotObj.transform.forward) * 9000f);
		
		//Otherwise, since the ray didn't hit anything, we're just going to guess and shoot the projectile in the general direction.
		} else {
			shotObj.GetComponent<Rigidbody>().AddForce((vRay.direction) * 9000f);
		}
